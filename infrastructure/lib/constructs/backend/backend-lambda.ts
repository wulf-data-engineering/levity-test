  ...
  return lambda.Code.fromAsset(workspacePath, {
    ...
    bundling: {
      image: cdk.DockerImage.fromRegistry(
        'ghcr.io/wulf-data-engineering/levity-lambda-builder:20251230',
      ),
      user: 'root',
      volumes: [
      ...
      local: {
        tryBundle(outputDir: string) {
          ...
          // cratePath is absolute (see line 96), so we use it directly.
          ...
          const binPath = path.join(
            buildDir,
            `target/aarch64-unknown-linux-gnu/release/${binName}`,
          );
...
function dynamicExclusions(include: string[], exclude: string[]): string[] {
  ...
  try {
    ...
    console.warn(
      `[CDK Warning] Could not read workspace path '${workspacePath}' to generate exclusions. Defaulting to empty list.`,
    );
    return [];
  }
}
